...
def get_arguments():
    ...
    parser.add_argument(
        '-v', '--vk', dest='vk_id', help='vk user ID (e.g. 44232785)',
        type=int, metavar='id', required=True
    )
    parser.add_argument(
        '-t', '--twitter', dest='twitter_id', help='Twitter user ID (e.g. 44)',
        type=int, metavar='id', required=True
    )
    parser.add_argument(
...
class MyApp(AddressBook):
    ...
    def get_friends(self):
        print('Getting VK friends...')
        self.friend_list = self._get_friends(self.vk_id, VK_API(), VK_Friend)
        print('Getting Twitter friends...')
        self.friend_list += self._get_friends(self.twitter_id, Twitter_API(),
                                              Twitter_Friend)
    ...
    def merge_friend_list(self):
        print('Merging friends...\r', end='')
        set_percentage = lambda x: print('{}%\r'.format(int(x)), end='')
